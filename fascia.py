...
picture_frame_data = [
    {'pts': pf_west_pts, 'bid': pf_west_id}, {'pts': pf_east_pts, 'bid': pf_east_id},
    {'pts': pf_nw_pts, 'bid': pf_nw_id}, {'pts': pf_ne_pts, 'bid': pf_ne_id}
]

# --- B. FASCIA ---
# Recessed under overhang. Outer face is at X=0, X=D, Y=L.
fascia_data = []
# Side Fascias
fascia_data.append(
...
for r in range(15):
    ...
    l_len = tx - (W + gap_rows) - (gap_boards / 2)
    infill_data.append(
        {'len': l_len, 'x': W + gap_rows, 'y': cy, 'bid': sm.allocate_garapa(l_len, f"R{r + 1} Left", "INFILL")})

    r_len = (D - W - gap_rows) - tx - (gap_boards / 2)
    infill_data.append({'len': r_len, 'x': tx + gap_boards / 2, 'y': cy,
                        'bid': sm.allocate_garapa(r_len, f"R{r + 1} Right", "INFILL")})

    # Allocate Sister Block from H3.2 stock
...
print("=" * 80)
print(f"{'PLANK ID':12} | {'LENGTH':10} | {'CUTS & USAGE'}")
print("-" * 80)
for b in sm.garapa_registry:
    print(f"{b['id']:12} | {b['orig']:7.1f}mm | {b['cuts'][0]['usage']} ({b['cuts'][0]['len']:.1f}mm)")
    for c in b['cuts'][1:]:
        print(f"{'':15} | {'':10} | {c['usage']} ({c['len']:.1f}mm)")
